...
async function fetchProfile(userId: string): Promise<{ user: any; packages: any[] } | null> {
  ...
  return { user: userRes.data, packages: pkgRes.data ?? [] };
}
              ...
              {user.favorite_mountains?.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {user.favorite_mountains.map((m: string) => (
                    <span key={m} className="rounded-full border border-white/15 bg-white/8 px-2.5 py-0.5 text-xs text-white/60">
                      ⛰ {m}
                    </span>
                  ))}
                </div>
